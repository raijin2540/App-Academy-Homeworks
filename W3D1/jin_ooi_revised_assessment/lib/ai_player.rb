...
class AIPlayer
  ...
  def favorite_suit
    ...
    # debugger
    suit_count.key(suit_count.values.max)
  ...
  def play_card(pile, card)
    #added exeception, added play logic
  ...
  def choose_card(pile)
    ...
    #finds non-eights and check for valid play, otherwise play the eight
  ...
  def play_turn(pile, deck)
    #finds valid play or draws until valid play is available and
    #plays that card.
